...
function generateObject(metadata, attributes = {}) {
	...
	attributes.src = `${process.env.STATIC_PATH ? process.env.STATIC_PATH : ''}${lowsrc[0].url
		}`;
	...
	const missingSizesErrorMessage = `Missing \`sizes\` attribute on eleventy-img shortcode from: ${originalSrc || attributes.src
		}`;
	...
	if (formats.length === 1) {
		...
		const srcsetAttrValue = Object.values(lowsrc)
			.map(
				(entry) =>
					`${process.env.STATIC_PATH ? process.env.STATIC_PATH : ''}${entry.srcset
					}`
			)
			.join(', ');
